...
<Tip>
Issues inside the quiz frame come from your quiz provider's servers, not AutoProctor. AutoProctor cannot read, modify, or fix anything inside the quiz.
</Tip>
...
Everything inside the iframe comes directly from the quiz provider's servers. AutoProctor wraps the proctoring layer around it but has no access to the quiz content itself.
...
| **Questions not loading or loading slowly** | The provider's servers are slow or the form is too large. Test the quiz URL in a normal browser tab outside AutoProctor. |
...
If you contact us about an issue inside the quiz frame, we will not be able to investigate it -- we do not have access to the provider's systems. Contact the quiz provider directly.
