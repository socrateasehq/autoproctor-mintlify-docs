...
O AutoProctor monitora os candidatos em tempo real por meio de múltiplos canais (dependendo das suas [configurações de supervisão](/proctoring-settings)):
...
- [Envios Individuais](/access-candidate-responses) — Revisando o relatório de um candidato em detalhes
- [Nenhum Rosto ou Múltiplos Rostos Detectados](/no-face-or-multiple-faces-detected) — Por que a detecção facial pode gerar falsos positivos
- [Infração Falsa de Troca de Aplicativo](/false-app-switch-alert) — Entendendo as sinalizações de infrações falsas
- [Boas Práticas para Criadores de Testes](/best-practices-for-exam-creators) — Dicas para aplicar testes supervisionados de forma eficaz
