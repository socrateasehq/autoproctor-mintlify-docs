...
Generate a unique test link for each candidate so a single link cannot be forwarded to unintended recipients. See [Unique URL per Candidate](/settings/advanced-settings#unique-url-per-candidate-elite) for setup.

### Additional Instructions

Add custom instructions to the candidate instructions page — useful for sharing specific rules, context, or guidelines before the test begins. See [Additional Instructions](/settings/advanced-settings#additional-instructions-elite) for details.
